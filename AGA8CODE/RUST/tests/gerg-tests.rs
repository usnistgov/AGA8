...
fn gerg_demo_example() {
    ...
    assert!(f64::abs(gerg_test.s - -38.57590392409089) < 1.0e-4);
    assert!(f64::abs(gerg_test.cv - 39.02948218156372 ) < 1.0e-4);
    assert!(f64::abs(gerg_test.cp - 58.45522051000366 ) < 1.0e-3);
    assert!(f64::abs(gerg_test.w - 714.4248840596024 ) < 1.0e-2);
